...
const ChannelSection = ({ setHandler, setting }) => (
    <>
        ...
        />
        <Button title="POS" active={setting === config.channel.pos} onClick={() => setHandler(config.channel.pos)} />
        <Button title="Czat" active={setting === config.channel.chat} onClick={() => setHandler(config.channel.chat)} />
    </>
);
